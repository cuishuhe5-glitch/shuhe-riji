    ...
    script_path.write_text(
        "\n".join(
            [
                ...
                "  local target_real source_real candidate candidate_real",
                "  target_real=\"$(app_realpath \"$APP_DST\")\"",
                "  source_real=\"$(app_realpath \"$APP_SRC\")\"",
                "  local roots=(\"$HOME/Applications\" \"/Applications\" \"$HOME/Downloads\" \"$HOME/Desktop\")",
                "  for root in \"${roots[@]}\"; do",
                "    [ -d \"$root\" ] || continue",
                "    while IFS= read -r candidate; do",
                "      [ -d \"$candidate\" ] || continue",
                "      candidate_real=\"$(app_realpath \"$candidate\")\"",
                "      if [ \"$candidate_real\" != \"$target_real\" ] && [ \"$candidate_real\" != \"$source_real\" ]; then",
                f"        echo \"$(date '+%Y-%m-%d %H:%M:%S') removing duplicate $candidate\" >> {shlex.quote(str(log_path))}",
                "        rm -rf \"$candidate\"",
                "      fi",
                "    done < <(find \"$root\" -maxdepth 3 -name '书赫日报助手.app' -type d 2>/dev/null)",
                "  done",
                "}",
                "stop_legacy_python_services() {",
                "  pkill -f '[p]ython.*-m riji' >/dev/null 2>&1 || true",
                "  pkill -f '[P]ython.*-m riji' >/dev/null 2>&1 || true",
                "}",
